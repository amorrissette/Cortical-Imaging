function plot_RegMovie(input,argin)
...
for x = 1:5:sZ
    ...
    subplot(1,3,2), imagesc(bsxfun(@times,input.blur3(:,:,x),mask)), caxis([-0.01 0.01]), title('Ratiometric'), axis off square
    ...
    subplot(1,3,3), plot(dataMean), hold on, line([x x],[dataMin, dataMax],'Color',[1 0 0]), axis square, title('ROI')
    pause(0.001)
    else 
        break
    end
