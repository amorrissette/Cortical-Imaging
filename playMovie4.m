function playMovie4(out, argsin)
...
Hem = bsxfun(@times,out.HemBlur,mask);
...
for x = 1:2:sZ
    if ishandle(h) == 1
        subplot(1,3,1), imagesc(out.imgD1(:,:,x)), caxis([0 8000]), colormap('parula'), title('Base'), axis off square
        subplot(1,3,2), imagesc(blur3(:,:,x)), caxis([-0.01, 0.005]), title('Voltage Signal'), axis off square 
        subplot(1,3,3), imagesc(Hem(:,:,x)), caxis([-0.05 0.05]), title('Ratiometric'), axis off square, hold on
