function out = preProcVSFP6(fDate, fNum, ~) 
...
[imgA1,imgD1,I] = stimCheck(imgD,Aphys,Dphys,sX,sY, sZ, hemfilter);
...
function [imgA1,imgD1,I] = stimCheck(imgD,Aphys,Dphys,sX,sY,sZ,hemfilter)
...
if length(I) >= 2
    ...
    sAhem = zeros(sX*sY,Li);
    sDhem = zeros(sX*sY,Li);
    pAhem = zeros(sX*sY,Lp);
    pDhem = zeros(sX*sY,Lp);

    stimA = reshape(imgA1(:,:,i(1):i(2)),[sX*sY,Li]);
    stimD = reshape(imgD1(:,:,i(1):i(2)),[sX*sY,Li]);
    preA = reshape(imgA1(:,:,1:I(1)),[sX*sY,Lp]);
    preD = reshape(imgD1(:,:,1:I(1)),[sX*sY,Lp]);
    for xy = 1:sX*sY
    
    % filter for hemodynamic signal (12-14 hz)
        sAhem(xy,:) = filtfilt(hemfilter.numer, hemfilter.denom,stimA(xy,:));
        sDhem(xy,:) = filtfilt(hemfilter.numer, hemfilter.denom,stimD(xy,:));
        pAhem(xy,:) = filtfilt(hemfilter.numer, hemfilter.denom,preA(xy,:));
        pDhem(xy,:) = filtfilt(hemfilter.numer, hemfilter.denom,preD(xy,:));
    end 

    % Standard deviation of filtered signal   
    stdStimA = std(sAhem,0,2);
    stdStimD = std(sDhem,0,2);    
    stdPreA = std(pAhem,0,2);
    stdPreD = std(pDhem,0,2);
    scaleA = reshape(stdPreA./stdStimA,[sX,sY]);
    scaleD = reshape(stdPreD./stdStimD,[sX,sY]);
    stimA1 = reshape(stimA,[sX,sY,Li]);
    stimD1 = reshape(stimD,[sX,sY,Li]);
    stimAnorm = bsxfun(@times,stimA1,scaleA);
    stimDnorm = bsxfun(@times,stimD1,scaleD);
    
    imgA1(:,:,i(1):i(2)) = stimAnorm;
    imgD1(:,:,i(1):i(2)) = stimDnorm;
    ...
    imgA1(:,:,i(1):i(2)) = bsxfun(@plus, Ai, stimAnorm);
    imgD1(:,:,i(1):i(2)) = bsxfun(@plus, Di, stimDnorm);
